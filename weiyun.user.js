// ==UserScript==
// @name         WeiyunHelper - 微云 Aria2 下载辅助脚本 (分支版，支持JSON-RPC)
// @namespace    https://github.com/loo2k/WeiyunHelper/
// @version      0.0.8-1
// @description  微云下载时文件支持导出到 Aria2 下载，支持分享页面及个人云盘管理页。此分支版改採 JSON-RPC 推送至 Aria2，並取消了 zip 打包下載，改支持一次性推送多檔。
// @author       Luke
// @author       PPRRPRR
// @match        *://*.weiyun.com/*
// @grant        GM_setValue
...
// @downloadURL  https://update.greasyfork.org/scripts/402669/WeiyunHelper%20-%20%E5%BE%AE%E4%BA%91%20Aria2%20%E4%B8%8B%E8%BD%BD%E8%BE%85%E5%8A%A9%E8%84%9A%E6%9C%AC.user.js
// @updateURL    https://update.greasyfork.org/scripts/402669/WeiyunHelper%20-%20%E5%BE%AE%E4%BA%91%20Aria2%20%E4%B8%8B%E8%BD%BD%E8%BE%85%E5%8A%A9%E8%84%9A%E6%9C%AC.meta.js
// @connect      *
// ==/UserScript==
...
//===============================================================================
// 請依照你的環境自行修改此地址變量。（目前沒有特別做一個UI方便你在頁面上設置...）
...
//===============================================================================
	 ...
	 * @param {void}
	 */
	function handleResp2Aria2(file_list=[])
	{
//		console.log('file_list.length:', file_list.length);

		// TODO: UI and GM_setValue('WYHLP:json-rpc-url');
		const jsonRpcUrl = GM_getValue('WYHLP:json-rpc-url') || JSON_RPC_URL;

		// 每個檔案的cookie不一樣，沒辦法合併一次送出給aria2c.addUri
		file_list.forEach( ({https_download_url, cookie_name, cookie_value})=>{
			const pathname = new URL(https_download_url).pathname;
			const out = decodeURI(pathname.substring(pathname.lastIndexOf('/') + 1));
	...
	location.host === 'www.weiyun.com' && webpackJsonp([7891], {
		[injectChunkId]: function (modules, exports, require) {
			...
			function downloadSelectedFiles() {
				...
				const fileNodes = Array.from(selected).map(item => item.__vue__.fileNode).filter((fileNode) => !fileNode.isDir());

				// fetchDownloadFileInfo() 要求 fileNodes 每次至多3個
				const jobs = [];
				for (let i = 0; i < fileNodes.length; i += 3) jobs.push(diskService.fetchDownloadFileInfo({ fileNodes: fileNodes.slice(i, i + 3) }));

				Promise.allSettled(jobs)
					.then((results) => {
						let list = [];
						results.forEach(({ status, value }) => {
							if ('fulfilled' != status) return;
							list = list.concat(value.file_list);
						});

						if (list.length != fileNodes.length) console.warn('TO-DO: 某些下載解析失敗');
						handleResp2Aria2(list);
					})
					.catch((e) => GM_notification({
						title: '解析時發生問題',
						text: e.msg,
						silent: true
						//			timeout: 7000
